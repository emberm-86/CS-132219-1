...
public class LogService {
  ...
  public void saveLogEventsToDatabase(List<LogEvent> logEvents) {
    convertLogEventsToDbEvents(logEvents).forEach(event -> {
  ...
  public List<Event> convertLogEventsToDbEvents(List<LogEvent> logEvents) {
    Map<String, Map<String, LogEvent>> logEventMap = logEvents.stream()
  ...
  private Event createEvent(Entry<String, LogEvent> startedLogEventEntry,
      Map<String, LogEvent> finishedMap) {
  ...
  private BiPredicate<LogEvent, LogEvent> logEventTypeCheck() {
    return (started, finished) -> (started.getType() == null && finished.getType() == null)
